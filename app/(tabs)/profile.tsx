...
function calculateCompletion(user: any) {
  ...
  const filled = fields.filter(
    (key) => user[key] && user[key] !== "" && user[key].length !== 0
  ).length;
...
export default function ProfileScreen() {
  ...
  const isVerified = completion >= 80; // ✅ dynamically verified if 80%+
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100 }}>
      ...
      <View style={styles.header}>
          ...
          <View style={{ position: "absolute" }}>
            <Svg width={radius * 2 + 10} height={radius * 2 + 10}>
              ...
              <AnimatedCircle
                ...
                cy={radius + 5}
                r={radius}
                stroke={isVerified ? "#10b981" : "#3b82f6"} // ✅ green if verified
                strokeWidth={strokeWidth}
        ...
        {/* ✅ Show badge only if completion ≥ 80% */}
        {isVerified && (
